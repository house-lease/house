...
public class PayApiController {
    ...
    @RequestMapping("/Pay")
    public String pay(HttpServletRequest request, HttpServletResponse response,String subject,String body,String total_amount,Integer userId)throws Exception{
    //                                                                                订单名称        商品描述     充值金额

        this.user = userService.queryByUserId(userId);

        //制造商户订单号 字符串形式 ：年份+月份+日期+小时+分钟+秒数+毫秒
        Date vNow = new Date();
        String sNow = ""+(vNow.getYear()+1900)+(vNow.getMonth()+1)+vNow.getDate()+vNow.getHours()+vNow.getMinutes()+vNow.getSeconds()+(vNow.getTime() / 1000);
...
//        }
        ...
        if(verify_result){//验证成功
            ...
            //设置user
            particular.setUser(this.user);
            //设置money{先将string转成long再将long专成BigDecimal}
            particular.setMoney(BigDecimal.valueOf(Long.getLong(params.get("total_amount"))));
            //设置充值时间
